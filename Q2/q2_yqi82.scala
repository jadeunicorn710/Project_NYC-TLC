...
// ENTER THE CODE BELOW
var df1a = df_filter.groupBy($"DOLocationID").agg(count($"DOLocationID") as "number_of_dropoffs").orderBy($"number_of_dropoffs".desc, $"DOLocationID".asc)
df1a.show(5)
...
// ENTER THE CODE BELOW
var df1b = df_filter.groupBy($"PULocationID").agg(count($"PULocationID") as "number_of_pickups").orderBy($"number_of_pickups".desc, $"PULocationID".asc)
df1b.show(5)
...
// ENTER THE CODE BELOW
var df2a = df1a.withColumnRenamed("DOLocationID", "LocationID")
var df2b = df1b.withColumnRenamed("PULocationID", "LocationID")
var df2c = df2a.join(df2b, Seq("LocationID"), "full").na.fill(0)
var df2 = df2c.groupBy("LocationID").agg(sum($"number_of_dropoffs" + $"number_of_pickups") as "number_activities").orderBy($"number_activities".desc, $"LocationID".asc)
df2.show(3)
...
// ENTER THE CODE BELOW
var df3a = df2.join(df_map, Seq("LocationID"), "full").na.fill(0)
var df3 = df3a.groupBy("Borough").agg(sum($"number_activities") as "total_number_activities").orderBy($"total_number_activities".desc)
df3.show()
...
// ENTER THE CODE BELOW
var df4a = df_filter.withColumn("pickup_datetime", to_date($"pickup_datetime"))
var df4b = df4a.groupBy("pickup_datetime").agg((count($"PULocationID")) as "daily_count").orderBy($"daily_count".desc)
var df4c = df4b.withColumn("pickup_datetime", date_format($"pickup_datetime", "EEEE")).withColumnRenamed("pickup_datetime", "day_of_week")
var df4 = df4c.groupBy("day_of_week").agg((sum($"daily_count") / count($"day_of_week")) as "avg_count").orderBy($"avg_count".desc)
df4.show(2)
...
// ENTER THE CODE BELOW
var df5a = df_filter.withColumn("pickup_datetime", hour($"pickup_datetime")).withColumnRenamed("pickup_datetime", "hour_of_day").withColumnRenamed("PULocationID", "LocationID")
var df5b = df5a.join(df_map, Seq("LocationID"), "full").na.fill(0)
// var df5c = df5b.groupBy("Zone").agg(count($"LocationID") as "zone_count").orderBy($"zone_count".desc)
var df5c = df5b.filter($"Borough" === "Brooklyn").orderBy($"hour_of_day".asc, $"LocationID".asc)
var df5d = df5c.groupBy("hour_of_day", "Zone").agg(count($"LocationID") as "zone_count").orderBy($"hour_of_day".asc, $"zone_count".desc)
// var df5e = df5d.over(Window.partitionBy($"hour_of_day").orderBy($"hour_of_day"))
var byHour = Window.partitionBy($"hour_of_day").orderBy($"zone_count".desc)
var df5e = df5d.withColumn("rn", row_number.over(byHour)).where($"rn" === 1).drop("rn").orderBy($"hour_of_day".asc, $"zone_count".desc)
// var df5f = df5e.groupBy("hour_of_day", "Zone").agg(count($"LocationID") as "zone_count").orderBy($"hour_of_day".desc)
var df5 = df5e.withColumnRenamed("zone_count", "max_count")
df5.show()
...
// ENTER THE CODE BELOW
var df6a = df_filter.withColumn("pickup_datetime", to_date($"pickup_datetime")).withColumnRenamed("PULocationID", "LocationID")
var df6b = df6a.join(df_map, Seq("LocationID"), "full").na.fill(0)
var df6c = df6b.filter($"Borough" === "Manhattan" && month($"pickup_datetime") === 1 && dayofmonth($"pickup_datetime") =!= 1)
var df6d = df6c.withColumn("pickup_datetime", dayofmonth($"pickup_datetime")).withColumnRenamed("pickup_datetime", "pickup_day")
var df6e = df6d.groupBy("pickup_day").agg(count($"LocationID") as "number_pickups").orderBy($"pickup_day".asc)
var byDay = Window.orderBy($"pickup_day".asc)
var df6f = df6e.withColumn("number_pickups_lastday", lag($"number_pickups", +1).over(byDay))
var df6g = df6f.withColumn("percent_change", round(((col("number_pickups") - col("number_pickups_lastday")) / col("number_pickups_lastday") * 100), 2)).orderBy($"percent_change".desc)
var df6 = df6g.drop("number_pickups").drop("number_pickups_lastday")
df6.show(3)
